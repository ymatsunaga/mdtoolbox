function [T, emission, pi_i, logL_history] = msmbaumwelchdb_parallel(data, T0, emission0, pi0_i, tolerance, max_iteration)
%% msmbaumwelch
...
while (check_convergence > TOLERANCE) && (count_iteration < max_iteration)
  %% E-step
  disp('msmforward');
  [logL, alpha, factor] = msmforward_parallel(data_cell, T0, emission0, pi0_i);
  disp('msmbackward');
  [~, beta] = msmbackward_parallel(data_cell, factor, T0, emission0, pi0_i);
  disp('others');
